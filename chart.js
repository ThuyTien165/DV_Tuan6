...
function loadData() {
    // Dòng này chỉ định d3 đọc file 'data_ggsheet.csv'
...
document.addEventListener('DOMContentLoaded', function() {
    loadData().then(() => {
        if (globalData && globalData.length) {
            ...
            d3.select("#title").html("❌ Không có dữ liệu hợp lệ. Vui lòng kiểm tra file 'data_ggsheet - data.csv'");
        }
    }).catch(error => {
        console.error('Error loading data:', error);
        d3.select("#title").html("❌ Lỗi khi tải dữ liệu. Vui lòng kiểm tra file 'data_ggsheet - data.csv'");
    });
});
...
buttons.forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    // gắn active cho button được click
